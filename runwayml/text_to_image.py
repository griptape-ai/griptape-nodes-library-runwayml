...
def validate_prompt_length(parameter, value):
    ...
    if value and len(str(value)) > MAX_PROMPT_LENGTH:
        raise ValueError(
            f"Prompt text must be {MAX_PROMPT_LENGTH} characters or less. Current length: {len(str(value))}"
        )
...
def validate_reference_tag(tag: str) -> str:
    ...
    if not re.match(r"^[a-zA-Z][a-zA-Z0-9_]*$", tag):
        raise ValueError(
            f"Tag '{tag}' is invalid. Must start with a letter and contain only letters, numbers, and underscores."
        )
...
class RunwayML_TextToImage(ControlNode):
    ...
    def validate_node(self) -> list[Exception] | None:
        ...
        for i, item in enumerate(ref_images_list):
            logger.info(
                f"RunwayML T2I validation: item {i + 1} type={type(item).__name__}, value={getattr(item, 'value', 'N/A')}"
            )
        ...
        for i, ref_artifact in enumerate(ref_images_list):
            ...
            if hasattr(ref_artifact, "image") and hasattr(ref_artifact, "tag"):
                ...
                hasattr(ref_artifact, "value")
                and isinstance(ref_artifact.value, dict)
                and "image" in ref_artifact.value
            ):
                ...
                logger.info(
                    f"RunwayML T2I validation: Reference image {i + 1} is a dictionary, extracting image and tag"
                )
                image = ref_artifact["image"]
                tag = ref_artifact.get("tag", "")  # Tag is optional
            else:
                logger.error(
                    f"RunwayML T2I validation: Reference image {i + 1} is type {type(ref_artifact).__name__}, expected object with image data"
                )
                errors.append(ValueError(f"Reference image {i + 1} must have image data."))
                continue

            logger.info(
                f"RunwayML T2I validation: Reference image {i + 1} - image type: {type(image).__name__}, tag: '{tag}'"
            )
            ...
            logger.info(
                f"RunwayML T2I validation: _get_image_data_uri returned: {bool(image_uri)} (length: {len(image_uri) if image_uri else 0})"
            )

            if not image_uri:
                logger.error(
                    f"RunwayML T2I validation: Failed to convert image {i + 1} to data URI. Image type: {type(image).__name__}"
                )
                ...
                if hasattr(image, "media_type"):
                    ...
                    if len(test_uri.encode("utf-8")) > 5 * 1024 * 1024:
                        errors.append(
                            ValueError(
                                f"Reference image {i + 1} (tag: '{tag}') is too large ({len(test_uri.encode('utf-8')) / (1024 * 1024):.1f}MB). RunwayML has a 5MB limit for data URIs (~3.3MB unencoded). Try using smaller images or HTTPS URLs."
                            )
                        )
                        continue
        ...
        def generate_image_async() -> ImageUrlArtifact | ErrorArtifact:
            try:
                ...
                if ref_images_list:
                    ...
                    for i, ref_artifact in enumerate(ref_images_list):
                        ...
                        if hasattr(ref_artifact, "image") and hasattr(ref_artifact, "tag"):
                            ...
                            # Dictionary representation (likely from serialization)
                            logger.info(
                                f"RunwayML T2I: Reference image {i + 1} is a dictionary, extracting image and tag"
                            )
                            image = ref_artifact["image"]
                            tag = ref_artifact.get("tag", "")  # Tag is optional
                        else:
                            logger.warning(
                                f"RunwayML T2I: Skipping reference image {i + 1} - type {type(ref_artifact).__name__} doesn't have required attributes"
                            )
                            continue

                        if not image:
                            logger.warning(
                                f"RunwayML T2I: Skipping reference image {i + 1} (tag '{tag}') - missing image"
                            )
                            continue
                        ...
                        logger.info(
                            f"RunwayML T2I: Processing reference image {i + 1} with tag '{tag}': image={type(image).__name__}"
                        )
                        ...
                        if image_uri:
                            ...
                            logger.info("RunwayML T2I: Failed to convert image URI")

                        if image_uri and tag and str(tag).strip():
                            processed_reference_images.append({"uri": image_uri, "tag": str(tag).strip()})
                            logger.info(f"RunwayML T2I: Added reference image with tag '{tag}'")
                        else:
                            logger.warning(
                                f"RunwayML T2I: Skipped reference image {i + 1} (tag '{tag}') - image_uri={bool(image_uri)}"
                            )
                ...
                task_payload = {
                    ...
                    "ratio": ratio_val,
                    "contentModeration": {"publicFigureThreshold": content_moderation},
                }
                ...
                if processed_reference_images:
                    ...
                    logger.info("RunwayML T2I: No valid reference images, not including referenceImages in payload")
                ...
                response = requests.post(
                    "https://api.dev.runwayml.com/v1/text_to_image", json=task_payload, headers=headers, timeout=60
                )
                ...
                for attempt in range(max_retries):
                    ...
                    logger.info(
                        f"RunwayML T2I generation status (Task ID: {task_id}): {status} (Attempt {attempt + 1}/{max_retries})"
                    )
                    ...
                    if status == "SUCCEEDED":
                        ...
                        if image_url:
                            ...
                            logger.error(
                                f"RunwayML T2I task SUCCEEDED but no output URL found. Output structure: {output}"
                            )
                ...
                self.publish_update_to_parameter(
                    "seed", actual_seed if "actual_seed" in locals() else RunwayML_TextToImage._last_used_seed
                )
